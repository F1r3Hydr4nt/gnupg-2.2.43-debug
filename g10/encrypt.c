...
{
  log_info(_("Encrypting session key\n"));
  gpg_error_t err;
...
void print_iobuf_info(const struct iobuf_struct *iobuf) {
        log_info("print_iobuf_info\n");
    if (!iobuf) {
        log_info("iobuf is NULL\n");
        return;
    }

    log_info("IOBUF Information:\n");
    log_info("  Use: %d\n", iobuf->use);
    log_info("  nlimit: %ld\n", (long)iobuf->nlimit);
    log_info("  nbytes: %ld\n", (long)iobuf->nbytes);
    log_info("  ntotal: %ld\n", (long)iobuf->ntotal);
...
encrypt_simple (const char *filename, int mode, int use_seskey)
{
  ...
  if ( do_compress )
    {
          log_info("do_compress %d",default_compress_algo());
