...
void print_context_info(const compress_filter_context_t *context) {
    log_info("Algorithm: %d\n", context->algo);
    log_info("Input buffer size: %u bytes\n", context->inbufsize);
    log_info("Output buffer size: %u bytes\n", context->outbufsize);
}
gpg_error_t
push_compress_filter(IOBUF out,compress_filter_context_t *zfx,int algo)
{
	    log_info("push_compress_filter");
		// print_context_info(zfx);
