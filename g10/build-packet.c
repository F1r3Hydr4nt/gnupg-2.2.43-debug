...
build_packet (IOBUF out, PACKET *pkt)
{
  ...
  log_info("CTB: pkttype %02x %d", ctb, pkttype);
  switch (pkttype)
    {
    case PKT_ATTRIBUTE:
    case PKT_USER_ID:
...
;
    ...
    log_info("  ntotal: %ld\n", (long)iobuf->ntotal);
    log_info("  nofast: %d\n", iobuf->nofast);
    log_info("  Buffer size: %zu\n", iobuf->d.size);
    log_info("  Buffer start: %zu\n", iobuf->d.start);
    log_info("  Buffer length: %zu\n", iobuf->d.len);
    log_info("  Filter EOF: %d\n", iobuf->filter_eof);
    log_info("  Error: %d\n", iobuf->error);
    log_info("  Filter owner: %d\n", iobuf->filter_ov_owner);
    if (iobuf->real_fname) {
        log_info("  Real filename: %s\n", iobuf->real_fname);
    } else {
        log_info("  Real filename: (null)\n");
    }
    log_info("  Filter number: %d\n", iobuf->no);
    log_info("  Sub filters count: %d\n", iobuf->subno);
...
do_symkey_enc( IOBUF out, int ctb, PKT_symkey_enc *enc )
{
 log_info("do_symkey_enc");
  ...
  // log_info("Writing s2k.count");
    ...
    // log_info("enc->seskeylen = %d", enc->seskeylen);
    if( enc->seskeylen )
...
void print_buffer(const char *buffer, size_t length) {
    char buffer_str[length * 3 + 1]; // each byte needs 2 hex digits plus a space, plus a null terminator
    size_t pos = 0;

    for (size_t i = 0; i < length; i++) {
        pos += sprintf(buffer_str + pos, "%02X ", (unsigned char)buffer[i]);
    }

    log_info("BUFFER: %s", buffer_str);
}
...
write_header2( IOBUF out, int ctb, u32 len, int hdrlen )
{
  ...
  if( len || hdrlen )
    ...
    log_info("WRITE HEADER:     ctb=%02x, len=%d", ctb, len);
...
{
  log_info("write_new_header %02X %08X %d",ctb,len,hdrlen);
    if( hdrlen )
